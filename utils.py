...
import math
"""
This file contains utility functions like reading a hdf5 dataset and 
sampling batches from given lists
"""
...
def next_batch(num, data, labels):
    ...
    for batch_no in batch_numbers:
        ...
        if not batch_is_used[batch_no]:
            ...
            # Find the indices and return batch
            return np.asarray(data[batch_no*num:(batch_no + 1)*num][:]),\
                   np.asarray(labels[batch_no*num:(batch_no + 1)*num][:])